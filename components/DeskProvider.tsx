...
import { DeskActionsContext, DeskStateContext, type DeletedItem } from './DeskActionsContext';
import type { 
  StickyNoteData, 
  DeskFileData, 
  DeskFolderData, 
...
export default function DeskProvider({ children }: DeskProviderProps) {
  const { user } = useAuth();
  ...
  const handleFolderDrop = (folder: DeskFolderData, target: string) => {
    // Implementation for folder drop handling
  };

  const handleStickyNoteDrop = (note: StickyNoteData, target: string) => {
    // Implementation for sticky note drop handling
  };

  const handleTornPageDrop = (page: TornPageData, target: string) => {
    // Implementation for torn page drop handling
  };
  ...
  const deskStateValue = {
    ...
    handleFileDrop,
    handleFolderDrop,
    handleStickyNoteDrop,
    handleTornPageDrop,
    handleFolderSelection,
    handleCreateFolderInTray,
    handleFileImported,
    handleLongTermFileImported,
    handleRestoreItem,
  ...
  return (
    <DeskActionsContext.Provider value={deskActionsValue}>
      <DeskStateContext.Provider value={deskStateValue}>
        {children}
